...
public class ZabbixDriver {
	...
	private static Logger LOGGER = LoggerFactory.getLogger(ZabbixDriver.class);
	...
	public ZabbixDriver(final String username, final String password, final String httpApiAddress,
			final String hostGroupName, final String templateName, final int port) {
		this.username = username;
		this.password = password;
		this.httpApiAddress = httpApiAddress;
	 ...
	 * @param template
	 */
	public void assignHostToHostgroup(String authToken, String host_name, String host_ip) throws MonitorException {
		try {
			JSONObject mainJObj = new JSONObject();
			JSONObject paramJObj = new JSONObject();
			...
			int template_id = getTemplate(authToken, templateName);
			int hg_id = getHostGroupIdByName(authToken, hostGroupName);
			mainJObj.put("jsonrpc", "2.0");
	...
	public Double usedMemory(String authToken, int hostId) throws MonitorException {
		Double used = 0.0d;
		int total = 0;
		int free = 0;

		total = totalMemory(authToken, hostId);
		free = availableMemory(authToken, hostId);
		if (total == 0) {
			return 0.0;
		}
		try {
			used = Double.parseDouble("" + ((total - free) / total) * 100);
		} catch (NumberFormatException ex) {
			LOGGER.error("Cannot convert int to double : " + ex.getMessage() + " on used memory % calculation ");
		}
		return used;
	}
